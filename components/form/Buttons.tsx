...
export function SubmitButton({className='',text='submit',size='lg'}:SubmitButtonProps){
  ...
  return (
    <Button type="submit" disabled={pending} className={`capitalize bg-primary ${className}`} size={size}>
      {
        pending?<>
        <ReloadIcon className="pr-2 h-4 w-4 animate-spin" />
        Please wait....
        </>:
        text
      }
...
// button for signInModal if the user is not loggedIn
